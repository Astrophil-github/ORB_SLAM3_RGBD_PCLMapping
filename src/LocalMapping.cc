 ...
 */
LocalMapping::LocalMapping(System* pSys, Atlas *pAtlas, shared_ptr<PointCloudMapping> pPointCloud, const float bMonocular, bool bInertial, const string &_strSeqName):
...
void LocalMapping::Run()
{
    ...
    // cv::Mat cvDepth(480,640,CV_16SC1);
    // cvDepth = mpAtlas->GetAllKeyFrames()[0]->AmvDepth;
    // cv::imshow("cvDepth", cvDepth);

    // cv::Mat cvImage(480,640,CV_16SC1);
    // cvImage = mpAtlas->GetAllKeyFrames()[0]->AmvImage;
    // cv::imshow("cvImage", cvImage);

    // cv::waitKey(0);


    std::vector<ORB_SLAM3::KeyFrame *> ApKF;
    ApKF = mpAtlas->GetAllKeyFrames();
    

    cout<<"处理稠密点云  "<< "ApKF.size(): "<<ApKF.size() <<endl;

    cv::Mat AmvDepth;
    cv::Mat AmvImage;

    for ( int i = 0 ; i < ApKF.size() ; i++ )
    {
        cout<<"点云帧处理进度: "<< i+1 << "/" << ApKF.size() << endl;
        if (ApKF[i]->mnId==0)       // mnID为0时，AmvDepth和AmvImage一般为空，所以退出，不然有时候会报错
        {
            continue;
        }
        // std::cout<<ApKF[i]->mnId<<std::endl;
        AmvDepth = ApKF[i]->AmvDepth.clone();
        AmvImage = ApKF[i]->AmvImage.clone();
        if (i==0)
        {
            mpPointCloudMapping->AinsertKeyFrame( ApKF[i], AmvImage, AmvDepth, true, false ); //修改地方
        }
        else if (i==ApKF.size()-1)
        {
            mpPointCloudMapping->AinsertKeyFrame( ApKF[i], AmvImage, AmvDepth, false, true ); //修改地方
        }
        else    
        {
