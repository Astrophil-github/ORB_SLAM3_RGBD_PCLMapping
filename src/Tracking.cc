...
Tracking::Tracking(System *pSys, ORBVocabulary* pVoc, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer,
    Atlas *pAtlas, shared_ptr<PointCloudMapping> pPointCloud, KeyFrameDatabase* pKFDB, const string &strSettingPath, const int sensor, Settings* settings, const string &_nameSeq)
...
void Tracking::CreateNewKeyFrame()
{
    ...
    if(mSensor==System::RGBD)
    {
        // pKF->AmvDepth.push_back(imDepth);
        // pKF->AmvImage.push_back(mImRGB);

        // cv::imshow("imDepth", imDepth);
        // cv::imshow("mImRGB", mImRGB);

        // cv::waitKey(0);

        pKF->AmvDepth=imDepth.clone();
        pKF->AmvImage=mImRGB.clone();
        // pKF->AmvKF.push_back(pKF);
    }
    ...
    // insert Key Frame into point cloud viewer         LX
    
    mpPointCloudMapping->insertKeyFrame( pKF, this->mImRGB, this->imDepth ); //修改地方
